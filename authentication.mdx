...
## Integration values issued by Keystone

Keystone issues two values for an integration.

| Value | Used as | Where to use it |
| --- | --- | --- |
...
- Use the Partner API key only for catalog operations: categories, products, product detail, search, and exchange quotes.
- Do not send `partnerId` on catalog requests. The API key already identifies which catalog to return.
- Treat the Partner ID as a non-secret identifier. Use it only where the API explicitly asks for `partnerId` or `x-partner-id`.
...
The customer token is issued after OTP verification. Send `x-partner-id` on requests that create or read commerce data, using the Partner ID issued by Keystone.
...
`partnerId` is required for every customer OTP verification. Use the Partner ID issued for your integration.
