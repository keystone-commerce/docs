...
Customer profile and address endpoints require a bearer token. The token already identifies the customer account, so these endpoints do not require `x-partner-id`.
